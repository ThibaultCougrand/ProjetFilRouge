...
class ModeleOneRecipe extends ClassConnexion{
    ...
    public function ingredientsList($id){
        $array = [];
        $req = parent::$bdd->prepare("SELECT ingredients.id, ingredients.name, qtx,unite FROM `rec_has_ing`JOIN ingredients ON rec_has_ing.id_ingredient=ingredients.id JOIN unit ON ingredients.id_unit=unit.id WHERE id_recipe=:id "); 
        $req->bindParam(':id',$id);
        $req->execute();
